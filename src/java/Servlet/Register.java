...
public class Register extends HttpServlet {
    ...
    private void procesoSQLpost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        boolean[] found = us.findUsuario(user, pass, email);
        if (found[0] == false && found[1] == false) {
            //Registro completado
            //Mirar error en ficchero
            boolean done=us.addUsuario(user, pass, email,null);
                String text = "Se ha registrado satisfactoriamente";
            ...
            if(found[0] == true && found[1] == true){
                String text = "Usuario y email ya en uso";
                response.setContentType("text/plain");  // Set content type of the response so that jQuery knows what it can expect.
                response.setCharacterEncoding("UTF-8"); // You want world domination, huh?
                response.getWriter().write(text);       // Write response body.
            }
            if (found[0] == true) {
                String text = "Usuario ya en uso";
                response.setContentType("text/plain");  // Set content type of the response so that jQuery knows what it can expect.
                response.setCharacterEncoding("UTF-8"); // You want world domination, huh?
                response.getWriter().write(text);       // Write response body.
            }
            if (found[1] == true) {
                String text = "Email ya en uso";
                response.setContentType("text/plain");  // Set content type of the response so that jQuery knows what it can expect.
                response.setCharacterEncoding("UTF-8"); // You want world domination, huh?
                response.getWriter().write(text);       // Write response body.
            }
